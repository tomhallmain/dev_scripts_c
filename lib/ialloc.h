...
IALLOC_INLINE void *_GL_ATTRIBUTE_COLD _gl_alloc_nomem(void) {
  errno = ENOMEM;
  return NULL;
}
...
void *reallocarray(void *p, idx_t n, idx_t s);

IALLOC_INLINE void *ireallocarray(void *p, idx_t n, idx_t s) {
  ...
  if (n == 0 || s == 0)
    n = s = 1;
  return (n <= SIZE_MAX && s <= SIZE_MAX ? reallocarray(p, n, s)
                                         : _gl_alloc_nomem());
}
