...
static void extend_if_necessary(map1 m);
 ...
 */
int map_size(const map1 m) { return m->size; }
 ...
 */
bool map_contains(const map1 m, const char *key) {
 ...
 */
void map_set(map1 m, const char *key, void *value) {
  int b = hash(key) % m->capacity;
 ...
 */
void *map_get(const map1 m, const char *key) {
  int b = hash(key) % m->capacity;
 ...
 */
void *map_remove(map1 m, const char *key) {
  int b = hash(key) % m->capacity;
 ...
 */
const char *map_next(map1 m, const char *key) {
