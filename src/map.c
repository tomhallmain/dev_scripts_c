...
void map_put(hashmap *m, void *key, uintptr_t value) {
  hashmap_set(m, key, sizeof(key), value);
}

void map_put_(hashmap *m, void *key, uintptr_t value) {
  hashmap_set(m, key, sizeof(key) - 1, value);
}
...
bool map_get_(hashmap *m, void *key, uintptr_t *out_val) {
  return hashmap_get(m, key, sizeof(key) - 1, out_val);
}
